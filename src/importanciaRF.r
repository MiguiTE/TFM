...
for (n in nvecinos){
    for (patron in opcionPatrones){
        impFinalOcc = matrix(0, ncol = 2, nrow = length(patrones[[patron]]))
        impFinalReg = matrix(0, ncol = 2, nrow = length(patrones[[patron]]))
        impOcc = matrix(0, ncol = 2, nrow = length(patrones[[patron]]))
        impReg = matrix(0, ncol = 2, nrow = length(patrones[[patron]]))
        indices = seq(1, length(patrones[[patron]]) * n + 1, n)
        if (ECMS) {
            for(gcm in gcms){
        ...
        if (PP){
            load(paste0(ruta, "data/proyeccion/modelos/interimpatron", patron, "nvecions",n, "RFImp.rda"))
                for (k in 1:length(modelos[["Occ"]])){
                    tmpOcc = modelos[["Occ"]][[k]][["importance"]][,c(3,4)]
                    tmpReg = modelos[["Reg"]][[k]][["importance"]][,]
                    for (i in 1:(length(indices) - 1)){
                ...
                colnames(impFinalOcc) = colnames(modelos[["Occ"]][[k]][["importance"]])[c(3,4)]
                colnames(impFinalReg) = colnames(modelos[["Reg"]][[k]][["importance"]])
                save(impFinalOcc, impFinalReg, file = paste0(ruta, "data/proyeccion/modelos/importanciaInterimpatron", patron, "nvecions",n, "RF.rda"))
        }
    }
}
