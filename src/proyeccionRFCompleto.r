...
load(paste0(ruta, "data/proyeccionInterimCompleto.rda", collapse = ""))
...
for (n in nvecinos){
    for (patron in opcionPatrones){
      for(gcm in gcms){
        ...
        load(paste0(ruta,"data/proyeccion",gcm,"Completo.rda", collapse = ""))
        ...
        #preparar datos
        trainReg = prepareData(prd, obs.reg, local.predictors = list(vars = patrones[[patron]], n = n))
        trainOcc = prepareData(prd, obs.occ, local.predictors = list(vars = patrones[[patron]], n = n))
        ...
        save(modelos, file = paste0(ruta, "data/proyeccion/modelos/modelos", gcm, "patron", patron, "nvecions",n, "RFCompleto.rda"))
        save(prediccionReg, prediccionOcc, prediccionFinal, file = paste0(ruta, "data/proyeccion/resultados/pred", gcm, "nvecions", n, "patron", patron, "RFCompleto.rda"))
