...
for(estacion in names(estaciones)){
  if(GUARDA){
    pdf(paste0(ruta, "imagenes/RV", estacion, "An.pdf", collapse = ""))  
  }
  tmp = lapply(modelos, function(modelo){
    do.call("<-",list("yRegPred", eval(parse(text = modelo))))
    tmp2 = lapply(1:length(yRegPred), function(i){
  ...
  tiempos = df[is.na(df$L3),]$value
  df = df[!is.na(df$L3),]
  boxplot(value ~ L1, data = df,ylim = c(0, 1), main=paste("RV", estacion), at = seq(1, length(modelos), by = 1), names = modelos, las = 2)
  abline(h=1, col = "grey", lty=3, lwd=2)
  abline(h=0, col = "grey", lty=3, lwd=2)
  if(GUARDA){
    dev.off()  
...
for(estacion in names(estaciones)){
  if(GUARDA){
    pdf(paste0(ruta, "imagenes/R95P", estacion, "An.pdf", collapse = ""))  
  }
  tmp = lapply(modelos, function(modelo){
    do.call("<-",list("yRegPred", eval(parse(text = modelo))))
    tmp2 = lapply(1:length(yRegPred), function(i){
  ...
  tiempos = df[is.na(df$L3),]$value
  df = df[!is.na(df$L3),]
  boxplot(value ~ L1, data = df,ylim = c(0, 1), main=paste("Ratio Percentil 95", estacion), at = seq(1, length(modelos), by = 1), names = modelos, las = 2)
  abline(h=1, col = "grey", lty=3, lwd=2)
  abline(h=0, col = "grey", lty=3, lwd=2)
  if(GUARDA){
    dev.off()  
...
for(estacion in names(estaciones)){
  if(GUARDA){
    pdf(paste0(ruta, "imagenes/Sesgo", estacion, "An.pdf", collapse = ""))  
  }
  tmp = lapply(modelos, function(modelo){
    do.call("<-",list("yRegPred", eval(parse(text = modelo))))
    tmp2 = lapply(1:length(yRegPred), function(i){
...
for(estacion in names(estaciones)){
  if(GUARDA){
    pdf(paste0("imagenes/RV", estacion, "Est.pdf", collapse = ""))
  }
  tmp = lapply(modelos, function(modelo){
    if (modelo == "GLM"){
      loadGLM2(estacion)
...
for(estacion in names(estaciones)){
  if(GUARDA){
    pdf(paste0("imagenes/R95P", estacion, "Est.pdf", collapse = ""))
  }
  tmp = lapply(modelos, function(modelo){
    if (modelo == "GLM"){
      loadGLM2(estacion)
...
for(estacion in names(estaciones)){
  if(GUARDA){
    pdf(paste0(ruta, "imagenes/Sesgo", estacion, "Est.pdf", collapse = ""))  
  }
  tmp = lapply(modelos, function(modelo){
    if (modelo == "GLM"){
      loadGLM2(estacion)
