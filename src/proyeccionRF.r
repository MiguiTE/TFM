...
for (patron in opcionPatrones){
  for(gcm in gcms){
    ...
    #preparar datos
    trainReg = prepareData(prd, obs.reg, local.predictors = list(vars = patrones[[patron]], n = 25))
    trainOcc = prepareData(prd, obs.occ, local.predictors = list(vars = patrones[[patron]], n = 25))
    ...
    save(modelos, file = paste0(ruta, "data/proyeccion/modelos/modelos", gcm, "patron", patron, "RF.rda"))
    save(prediccionReg, prediccionOcc, prediccionFinal, file = paste0(ruta, "data/proyeccion/resultados/pred", gcm, "patron", patron, "RF.rda"))
